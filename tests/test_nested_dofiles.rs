...
fn test_nested_success() {
    // Nested do-file that succeeds. Real Stata output from
    // tests/log-analysis/13_nested_success.do, committed verbatim.
    let log_path = PathBuf::from("tests/fixtures/logs/13_nested_success.log");
...
fn test_nested_error() {
    // Nested do-file with error in helper. Real Stata output from
    // tests/log-analysis/14_nested_error.do, committed verbatim.
    //
    ...
    let log_path = PathBuf::from("tests/fixtures/logs/14_nested_error.log");
    let errors = parse_log_file(&log_path).expect("Failed to parse log");
